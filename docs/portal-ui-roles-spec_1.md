...
**Version:** 1.2 · **Date:** July 6, 2026 · **Status:** Approved for development (decisions D1, D2, D6 incorporated)
...
### 1.1 Naming decision (important — updated in D6)

The word "provider" means two distinct things in this product and they must never be merged:

1. **The `providers` table** — bookable clinicians: calendars, availability, appointments. Code and tickets refer to these as **clinicians**.
2. **The provider user role** — the doctor or nurse who runs a clinic's portal account. Internal role key stays **`clinic_admin`** everywhere in code, RLS, and app_metadata (renaming a live role key invites auth bugs for zero benefit); the **UI label is "Provider"**.

Often the same human — Dr. Jones both logs in and gets booked — but not always: a nurse may run the clinic without being bookable, and a clinic may have three bookable clinicians and one login. Therefore: add an optional `user_id uuid references auth.users` column to the `providers` table (nullable, unique). When set, the portal can show that user a "My appointments" view and default new campaigns to their own calendar. When null, the clinician simply has no login. This column ships in Phase 3 with the clinicians editor.
...
| **Clinic Admin** *(UI label: "Provider")* | One clinic | Runs the clinic's outreach operation: patients, clinicians & availability, campaigns, clinic settings, and its own Staff users. |
...
**Decided by product owner (July 5–6, 2026):**

* **D6 — Operating model & onboarding.** There is exactly ONE platform Admin (the product owner), who onboards every clinic. Within a clinic the roles are Provider (doctor/nurse; internal key `clinic_admin`) and Staff. Onboarding runbook, all via `admin-manage`: (1) Admin `create_clinic` with name, timezone, callback number; (2) Admin `create_user` role `clinic_admin` + that clinic_id, hands the temporary password to the provider out-of-band; (3) provider signs in, is forced to change the password, then self-serves: adds clinicians & availability, staff users, patients, and campaigns. The Admin never needs to touch a clinic again after step 2. UI consequence: the single-admin reality makes the last-admin lockout guards in `admin-manage` (no self-deactivation, no last-admin demotion) mandatory, not defensive.
